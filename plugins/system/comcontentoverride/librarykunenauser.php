...
class KunenaUser extends JObject {
	...
	public function getRank($catid = 0, $type = false) {
		$user_groups = $this->getUsersGroups();
		// Default rank
		$rank = new stdClass ();
		$rank->rank_id = false;
		...
		if (! $config->showranking)
			return;
		if (self::$_ranks === null) {
			$this->_db->setQuery ( "SELECT * FROM #__usergroups" );
			self::$_ranks = $this->_db->loadObjectList ();
		...
		if ($rank->rank_id === false) {
			...
			$rank->rank_id = 0;
			foreach ( self::$_ranks as $cur ) {
				if(in_array($cur->id, $user_groups) && ($cur->parent_id == $rank->rank_id || $rank->rank_id == 0)){

					$rank->rank_id = $cur->id;
					$rank->rank_title = $cur->title;
					$rank->parent_id = $cur->parent_id;
					$rank->rank_min = 0;
					$rank->rank_special = 0;
					$rank->rank_image = null;
				}
